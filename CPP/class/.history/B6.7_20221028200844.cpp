...
public:
    ...
    void ReplaceChar(char sub, char replace);           //替换所有字符(替换前的字符，替换后的字符)
    void ReplaceString(MyString sub, MyString replace); //替换子串(替换前的子串，替换后的子串)
};
...
{
    if(IsSubstring(sub) == false) return ;
    else
