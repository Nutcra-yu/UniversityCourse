...
public:
    //四个构造函数
    MyString() : string() {}
    MyString(const string &a)  { s = string(a); }
    MyString(int n, char a)  { s = string(n, a); }
    MyString(const string &a, int start, int length)  { s = string(a, start, length); }
    ...
    void Erase(char sub);   //擦除所有特定字符
    void Erase(string sub); //擦除首个特定子串
};

int main()
{
    /*构造函数测试*/
    MyString s1("nihao",100), s2(4, 'f'), s3("0123456789", 1, 9), s4;
    // s1.Show();
    // s2.Show();
    // s3.Show();
    // s4.Show();
...
//擦除首个特定子串
void MyString::Erase(string sub)
{
    if (IsSubstring(sub) == false)
        return;
    else
